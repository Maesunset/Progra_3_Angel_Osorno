En este texto se vera un código documentado de un árbol:
````c++
#ifndef AVL_TREE_H  
#define AVL_TREE_H  
#include<iostream>  
...
private:  
    ...
    // rotacion derecha  
    // guarda un nodo derecho como x y uno izquierdo como y    // actualiza el tamaño de ambas    std::shared_ptr<Node<T>> rightRotate(std::shared_ptr<Node<T>> y)  
    {  
    ...
    // rotacion izquierda  
    // guarda el nodo x como y, y el y como t2    // actualiza el tamaño de ambas    std::shared_ptr<Node<T>> leftRotate(std::shared_ptr<Node<T>> x)  
    {  
